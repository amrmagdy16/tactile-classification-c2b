...
5-Fold Cross-Validation with Early Stopping and MODEL SAVING.
"""
...
def run_fold(Xtr, ytr, Xte, yte, nc, device, epochs, lr, patience, fold_idx, save_dir):
    ...
    for ep in range(epochs):
        ...
        if te_acc > best_te:
            ...
            torch.save(model.state_dict(), os.path.join(save_dir, f"fold_{fold_idx}.pth"))
        else:
            bad += 1
            if bad >= patience:
                break
...
def main():
    ...
    save_dir = f"saved_models/attention_{args.condition}_{args.lr}_{args.epochs}"
    ...
    for fold, (tr, te) in enumerate(skf.split(X, ye), 1):
        tr_acc, te_acc, preds = run_fold(X[tr], ye[tr], X[te], ye[te], len(mats), device, 
                                         args.epochs, args.lr, patience=8, 
                                         fold_idx=fold, save_dir=save_dir)
    ...
    metadata = {
        ...
        'fold_accuracies': te_accs,
        'mean_test': float(np.mean(te_accs)),
        'std_test': float(np.std(te_accs)),
        'mean_train': float(np.mean(tr_accs)),
