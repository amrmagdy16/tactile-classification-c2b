#!/usr/bin/env python3
"""
[3/5] Attention CNN-LSTM + WEIGHT DECAY 1e-3.
Built on run_05 honest evaluation (validation-based early stopping; test fold
never used for selection). Only change vs run_05: Adam weight_decay raised 1e-4 -> 1e-3 (stronger L2 regularization).

Usage:
  python scripts_2/run_08_weightdecay.py --condition all     --exclude soft_bottle
  python scripts_2/run_08_weightdecay.py --condition press   --exclude soft_bottle
  python scripts_2/run_08_weightdecay.py --condition airhold --exclude soft_bottle
"""
...
def main():
    ...
    print("="*64); print(f"[3/5 WEIGHTDECAY] honest attention  condition={args.condition}  device={device}"); print("="*64)
    ...
    json.dump({'variant':'weightdecay','condition':args.condition,'mean_test':float(np.mean(te_a)),
               'std_test':float(np.std(te_a)),'mean_train':float(np.mean(tr_a)),
               'gap':float(np.mean(tr_a)-np.mean(te_a))},
              open(f"notes_3/reg_weightdecay_{args.condition}.json","w"),indent=2)
    print(f"Saved notes_3/reg_weightdecay_{args.condition}.json"); print("="*64)
