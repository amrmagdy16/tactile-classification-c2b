#!/usr/bin/env python3
"""
[4/5] Attention CNN-LSTM SHRUNK (uni-LSTM, hidden 32, 1 attention head).
Built on run_05 honest evaluation (validation-based early stopping; test fold
never used for selection). Only change vs run_05: Reduced capacity: LSTM unidirectional, hidden 32 (emb 32), attention 1 head.

Usage:
  python scripts_2/run_09_small.py --condition all     --exclude soft_bottle
  python scripts_2/run_09_small.py --condition press   --exclude soft_bottle
  python scripts_2/run_09_small.py --condition airhold --exclude soft_bottle
"""
...
class AttnCNNLSTM(nn.Module):
    def __init__(self, nc, ch=4):
        ...
        self.lstm = nn.LSTM(self.fd, 32, batch_first=True, bidirectional=False)
        self.self_attn = nn.MultiheadAttention(embed_dim=32, num_heads=1, batch_first=True)
        self.head = nn.Sequential(nn.Dropout(0.5), nn.Linear(32, nc))
    def forward(self, x):
        B,T,C,H,W = x.shape
...
def main():
    ...
    print("="*64); print(f"[4/5 SMALL] honest attention  condition={args.condition}  device={device}"); print("="*64)
    ...
    json.dump({'variant':'small','condition':args.condition,'mean_test':float(np.mean(te_a)),
               'std_test':float(np.std(te_a)),'mean_train':float(np.mean(tr_a)),
               'gap':float(np.mean(tr_a)-np.mean(te_a))},
              open(f"notes_3/reg_small_{args.condition}.json","w"),indent=2)
    print(f"Saved notes_3/reg_small_{args.condition}.json"); print("="*64)
