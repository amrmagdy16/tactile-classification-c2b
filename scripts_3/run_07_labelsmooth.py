#!/usr/bin/env python3
"""
[2/5] Attention CNN-LSTM + LABEL SMOOTHING 0.1.
Built on run_05 honest evaluation (validation-based early stopping; test fold
never used for selection). Only change vs run_05: CrossEntropyLoss(label_smoothing=0.1) to curb over-confident memorization.

Usage:
  python scripts_2/run_07_labelsmooth.py --condition all     --exclude soft_bottle
  python scripts_2/run_07_labelsmooth.py --condition press   --exclude soft_bottle
  python scripts_2/run_07_labelsmooth.py --condition airhold --exclude soft_bottle
"""
...
def main():
    ...
    print("="*64); print(f"[2/5 LABELSMOOTH] honest attention  condition={args.condition}  device={device}"); print("="*64)
    ...
    json.dump({'variant':'labelsmooth','condition':args.condition,'mean_test':float(np.mean(te_a)),
               'std_test':float(np.std(te_a)),'mean_train':float(np.mean(tr_a)),
               'gap':float(np.mean(tr_a)-np.mean(te_a))},
              open(f"notes_3/reg_labelsmooth_{args.condition}.json","w"),indent=2)
    print(f"Saved notes_3/reg_labelsmooth_{args.condition}.json"); print("="*64)
