...
VERSION 2: Per-Trial Normalization (Force-Invariant Features)

By dividing each trial's arrays by its peak deformation, we strip away
the human variance in pressing force and force the ML model to learn
the underlying material physics (curve shape, shear ratio, area spread).
"""
...
# ---------------------------------------------------------------
# FEATURE EXTRACTION (V2: FORCE-INVARIANT)
# ---------------------------------------------------------------
def extract_features(def_data, shear_data):
    """Extract 18 features normalized per-trial."""
    ...
    # === THE V2 NORMALIZATION TRICK ===
    # Find the maximum deformation value for THIS specific trial
    trial_max_def = def_ts[peak] + 1e-8  # Add tiny epsilon to avoid div-by-zero

    # Normalize time-series so peak is always 1.0
    def_ts_norm = def_ts / trial_max_def
    shear_ts_norm = shear_ts / trial_max_def # Normalize shear by def to keep Poisson ratio intact

    # Normalize the spatial 2D arrays at the peak frame
    def_peak = def_mag[peak] / trial_max_def
    shear_peak = shear_mag[peak] / trial_max_def

    # static deformation (Now measuring shape/distribution, not raw force)
    f_def_mean = np.mean(def_peak)
    f_def_std  = np.std(def_peak)
    f_def_max  = np.max(def_peak)
    f_def_p90  = np.percentile(def_peak, 90)
    
    # static shear (Relative to deformation)
    f_sh_mean = np.mean(shear_peak)
    f_sh_std  = np.std(shear_peak)
    f_sh_max  = np.max(shear_peak)
    f_sh_p90  = np.percentile(shear_peak, 90)

    # dynamic slopes (loading phase) - Using normalized curve!
    def slope(ts, p):
        if p > 5:
            seg = ts[max(0, p-20):p]
            if len(seg) > 2:
    ...
    f_def_slope = slope(def_ts_norm, peak)
    f_sh_slope  = slope(shear_ts_norm, peak)

    # time-series stats (using normalized TS)
    f_def_ts_mean = np.mean(def_ts_norm)
    f_def_ts_std  = np.std(def_ts_norm)
    f_sh_ts_mean  = np.mean(shear_ts_norm)
    f_sh_ts_std   = np.std(shear_ts_norm)

    # ratios
    f_ratio_mean = f_def_mean / (f_sh_mean + 1e-8)
    f_ratio_max  = f_def_max / (f_sh_max + 1e-8)

    # active area (Dynamic threshold: > 10% of the peak value, instead of absolute 0.1)
    f_def_area = np.mean(def_peak > (0.1 * f_def_max))
    f_sh_area  = np.mean(shear_peak > (0.1 * f_sh_max))
...
def main():
    print("="*60)
    print("C2b PIPELINE TEST - V2: FORCE-INVARIANT")
    print("="*60 + "\n")
